...
class Device extends Homey.Device {
  ...
  */
  ...
  async syncSettings(data) {
    ...
    for (const [name, value] of Object.entries(this.getSettings())) {
      ...
      if (name === 'ip_address' && value === 'undefined') continue;
    ...
    // Update settings
    if (filled(settings)) {
      this.log('[Sync] Updating settings', JSON.stringify(settings));
  ...
  */
  ...
  async setNetwork() {
    const address = this.getAddress();
    if (blank(address) || address === 'undefined') return;

    // Update settings
    if (this.getSetting('ip_address') !== address) {
      this.log(`[Network] Update IP address in settings to ${address}`);
  ...
  */
  ...
  getDiscoveredAddress() {
    return filled(this.discoveredAddress) ? this.discoveredAddress : null;
  }

  // Return IP address
  getAddress() {
    const manual = this.getManualAddress();

    if (filled(manual)) {
      this.log('[Address] Using manual address from settings', manual);
      return manual;
    }

    const discovered = this.getDiscoveredAddress();

    if (filled(discovered)) {
      this.log('[Address] Using discovered address', discovered);
      return discovered;
    }

    const settingIP = this.getSetting('ip_address');

    if (filled(settingIP) && settingIP !== 'undefined') {
      this.log('[Address] Using address from setttings', settingIP);
      return settingIP;
    }

    this.log('[Address] No usable address found');
    return null;
  }
