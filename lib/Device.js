...
class Device extends Homey.Device {
  ...
  static SYNC_INTERVAL = 25; // Seconds
  ...
  */
  ...
  async onInit() {
    ...
    // Add new capabilities if not present (for existing devices)
    const newCapabilities = ['meter_power', 'meter_power.today', 'meter_power.hour'];
    for (const cap of newCapabilities) {
      if (!this.hasCapability(cap)) {
        await this.addCapability(cap);
        this.log(`Added ${cap} capability`);
      }
    }

    // Remove deprecated capabilities from existing devices
    const deprecatedCapabilities = ['special_mode', 'auto_scan', 'scan_byte', 'scan_bit'];
    for (const cap of deprecatedCapabilities) {
      if (this.hasCapability(cap)) {
        try {
          await this.removeCapability(cap);
          this.log(`Removed deprecated ${cap} capability`);
        } catch (err) {
          this.log(`Could not remove ${cap} capability: ${err.message}`);
        }
      }
    }
  ...
  */
  ...
  async syncAccount() {
    ...
    if (this.accounts < 4) {
      ...
      // Account limit reached - try to register anyway (may replace old account)
  ...
  async queue(properties) {
    ...
    // Copy all properties to airconStat
  ...
  async updateDevice() {
    try {
      ...
      this.log('[Update] Full:', JSON.stringify(this.airconStat));
  ...
  */
  ...
  getCapabilityValuesFromStats() {
    ...
    // Update energy tracking with daily energy from device
    const dailyEnergy = this.airconStat.electric;
    this.updateEnergyTracking(dailyEnergy);
    ...
    return {
      ...
      meter_power: this.getTotalEnergy(),
      'meter_power.today': dailyEnergy,
      'meter_power.hour': this.getHourEnergy(),
