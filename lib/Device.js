...
class Device extends Homey.Device {
  ...
  */
  ...
  // Update device
  async updateDevice(property, value) {
    ...
    try {
      ...
      // Set property
      this.airconStat[property] = value;

      this.log(`-- AirconStat property '${property}' is now '${value}'`);
  ...
  */
  ...
  async onDiscoveryAvailable(result) {
    ...
    this.log('Available', `${result.address}:${result.port}`);

    // Update client address and port
    this.client.address = result.address;
    this.client.port = result.port;
  ...
  onDiscoveryAddressChanged(result) {
    this.log('Address changed', `${result.address}:${result.port}`);
