...
class Device extends Homey.Device {
  ...
  */
  ...
  // Synchronize timer
  async onSync() {
    if (this.state || this.client.lock.isLocked()) return;

    try {
      const data = await this.client.getAirconStat();
  ...
  async syncCapabilities() {
    ...
    for (const name of this.getCapabilities()) {
      const old = this.getCapabilityValue(name);
      ...
      this.log(`Device changed capability '${name}' from '${old}' to '${value}'`);
  ...
  async queue(properties) {
    ...
    this.log('[Queue] Current updates:', this.updates);
  ...
  async updateDevice() {
    try {
      ...
      await this.client.lock.waitForUnlock();
  ...
  */
  ...
  registerTimer(log = false) {
    ...
    this.syncDeviceTimer = this.homey.setInterval(this.onSync.bind(this), interval);
