...
class Device extends Homey.Device {
  ...
  */
  ...
  async syncCapabilityValues(data) {
    ...
    this.log('[Sync] Energy:', JSON.stringify({
      lastHour: this.lastHour,
      hourStartEnergy: this.hourStartEnergy,
      lastDailyEnergy: this.lastDailyEnergy,
      currentDailyEnergy: data.daily_energy,
      lastDay: this.lastDay,
      totalEnergy: this.totalEnergy,
      meter_power: data.meter_power,
    }));
  ...
  async syncEnergy(data) {
    if (!('daily_energy' in data)) return;
    if (data.daily_energy < 0) return;
    ...
    const currentDay = timeParts.find(part => part.type === 'day').value;
    ...
    if (filled(this.lastHour) && this.lastHour !== currentHour) {
      ...
      this.hourStartEnergy = data.daily_energy;
    ...
    if (blank(this.lastDay)) {
      this.hourStartEnergy = data.daily_energy;
      this.log(`[Energy] First run. Day: ${currentDay}, hour: ${currentHour}, daily: ${data.daily_energy} kWh`);
    }

    // Update tracking variables
    this.lastDay = currentDay;
    this.lastHour = currentHour;
    this.lastDailyEnergy = data.daily_energy;
