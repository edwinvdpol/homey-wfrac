...
class Device extends Homey.Device {
  ...
  */
  ...
  async sync() {
    ...
    try {
      ...
      await this.syncEnergyConsumption(data);
  ...
  async syncCapabilityValues(data) {
    ...
    data.meter_power = this.totalEnergy + this.lastDailyEnergy;

    // Get energy consumed this hour
    data['meter_power.hour'] = Math.max(0, this.lastDailyEnergy - this.hourStartEnergy);
  ...
  async syncEnergyConsumption(data) {
    if (!('meter_power.today' in data)) return;

    const dailyEnergy = data['meter_power.today'];

    this.log('[Energy] Syncing energy consumption');
    this.log('[Energy] Daily energy:', dailyEnergy, 'kWh');

    if (dailyEnergy < 0) return;
    ...
    if (filled(this.lastDay) && this.lastDay !== currentDay) {
    ...
    // Detect new hour
    if (filled(this.lastHour) && this.lastHour !== currentHour) {
  ...
  */
  ...
  async migrate() {
    ...
    // Add energy capabilities
    for (const name of ['meter_power', 'meter_power.today', 'meter_power.hour']) {
      if (this.hasCapability(name)) continue;

      await this.addCapability(name);
      this.log(`[Migrate] Added ${name} capability`);
    }
  ...
  */
  ...
  initEnergyConsumptions() {
    ...
    this.log(`[Energy] Initialized - Total: ${this.totalEnergy}, last day: ${this.lastDay}, last hour: ${this.lastHour}`);
  }
