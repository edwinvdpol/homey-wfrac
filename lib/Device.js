  ...
  */
  ...
  async onSettings({ oldSettings, newSettings, changedKeys }) {
    ...
    for (const name of changedKeys) {
      this.log(`[Settings] User changed '${name}' to '${newSettings[name]}'`);
    }
  ...
  */
  ...
  async syncSettings(data) {
    ...
    for (const [name, value] of Object.entries(this.getSettings())) {
      ...
      this.log(`[Sync] Device changed setting '${name}' to '${data[name]}'`);
    }
  ...
  async syncStore(data) {
    ...
    for (let [name, value] of Object.entries(this.getStore())) {
      ...
      this.log(`[Sync] Device changed store '${name}' to '${data[name]}'`);
    }
