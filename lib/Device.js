  ...
  */
  ...
  async onDiscoveryAvailable(result) {
    ...
    if ('address' in result) {
      await this.setSettings({ ip_address_discovered: result.address });
    }
  ...
  onDiscoveryAddressChanged(result) {
    ...
    if ('address' in result) {
      this.setSettings({ ip_address_discovered: result.address }).catch(this.error);
    }
  ...
  */
  ...
  async migrate() {
    ...
    if ('airconId' in store) {
      settings.aircon_id = store.airconId;
      this.unsetStoreValue('airconId').catch(this.error);
    }

    if ('operatorId' in store) {
      settings.operator_id = store.operatorId;
