...
class Device extends Homey.Device {
  ...
  */
  ...
  async sync(data = null) {
    ...
    try {
      ...
      this.error('[Sync]', JSON.stringify(err));
  ...
  async syncAirconStat(data = null) {
    if (!data) {
      this.log('[Client]', JSON.stringify(this.client.agent.getCurrentStatus()));
  ...
  */
  ...
  logPrefix() {
    ...
    return [
      `[FW:${settings.firmware_type || ''}]`,
      `[WIFI:${settings.wifi_firmware || ''}]`,
      `[MCU:${settings.mcu_firmware || ''}]`,
      `[ACC:${settings.accounts || ''}]`,
      `[IP:${settings.ip_address || ''}:${settings.port || ''}]`,
      `[ID:${this.getData().id}]`,
    ];
  }
