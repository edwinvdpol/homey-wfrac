...
class Client {
  ...
  async call(command, contents = null) {
    ...
    try {
      ...
      const res = await fetch(`${this.protocol}://${this.address}:51443/beaver/command/${command}`, {
        method: 'POST',
  ...
  async setProtocol() {
    ...
    try {
      ...
      await fetch(`http://${this.address}:51443/beaver/command/getDeviceInfo`, {
        method: 'POST',
    ...
    try {
      ...
      await fetch(`https://${this.address}:51443/beaver/command/getDeviceInfo`, {
        method: 'POST',
        body: JSON.stringify(this.getBody('getDeviceInfo')),
        agent: this.agent(),
      });
